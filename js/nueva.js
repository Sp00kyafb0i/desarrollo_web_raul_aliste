...
//Poblar el primer selector
var data = region_comuna.regiones
for(x in data) {
    var sel = document.createElement("option");
    sel.innerHTML = data[x].nombre;
    sel.value = data[x].nombre;
    document.getElementById("regiones").appendChild(sel);
    var data2 = data[x].comunas;
}
//Funcion para poblar el segundo selector
function regionChange() {
    var actual = document.getElementById("regiones").value;
    document.getElementById("comunas").innerHTML = "";
    for (x in data) {
        if (data[x].nombre == actual) {
            var data2 = data[x].comunas;
            console.log("Hit")
            for (y in data2) {
                var sel2 = document.createElement("option");
                sel2.innerHTML = data2[y].nombre;
                sel2.value = data2[y].nombre;
                document.getElementById("comunas").appendChild(sel2);
            }
        }
    }
}
